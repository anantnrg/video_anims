...
export default makeScene2D(function* (view) {
  ...
  yield* view.add(
    <Rect ref={input_data_ref} x={-700} y={-200} opacity={0}>
      <Icon icon={"mdi:file-document-multiple"} width={70} color={"cdd6f4"} />
    </Rect>,
  );

  yield* view.add(
    <Rect ref={function_working_ref} opacity={0} y={-90}>
      <Icon icon={"mdi:cog"} width={90} color={"cdd6f4"} />
    </Rect>,
  );

  yield* view.add(
    <Rect ref={output_data_ref} opacity={0} x={700} y={-50}>
  ...
  yield* chain(
    ...
    inputs_line_ref().points([[200, 0], Vector2.zero], 0.75, easeInOutQuad),
    ...
    outputs_line_ref().points([[200, 0], Vector2.zero], 0.75, easeInOutQuad),
